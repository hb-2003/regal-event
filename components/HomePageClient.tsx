...
type Props = {
  initialMosaicImgs: string[];
};

export default function HomePageClient({ initialMosaicImgs }: Props) {
  const [categories, setCategories] = useState<Category[]>([]);
  const [categoriesLoading, setCategoriesLoading] = useState(true);
  const [gallery, setGallery] = useState<GalleryItem[]>([]);
  const [lightbox, setLightbox] = useState<string | null>(null);
  const [mosaicImgs, setMosaicImgs] = useState<string[]>(initialMosaicImgs);
  const heroTexture =
    mosaicImgs.find((src) => src.trim().length > 0) ?? DEFAULT_HERO_TEXTURE;
  ...
  useEffect(() => {
    ...
    fetch("/api/settings", { cache: "no-store" })
      .then((r) => r.json())
      .then((d) => {
        const resolved = resolveHomeMosaicImages(
          d.home_hero_images,
          DEFAULT_HOME_MOSAIC
        ).map((src) => resolveDisplayImageSrc(src));
        setMosaicImgs(resolved);
      })
      .catch(() => {});
  }, []);
    ...
    <>
      ...
      >
        <div ref={heroBgRef} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', transform: 'scale(1.15)' }}>
          <HeroWebGL textureUrl={heroTexture} />
        </div>
